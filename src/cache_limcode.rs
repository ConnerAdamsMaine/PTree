...
impl LimcodeCache {
    ...
    /// Batch SIMD deserialization: get all entries using sequential batch processing
    /// Processes entries sequentially from sorted offsets, leveraging cache locality
    pub fn get_all_batch(&self) -> Result<Vec<LimcodeDirEntry>> {
        ...
        // Process entries in sorted offset order for cache locality
        for &offset in &self.index.sorted_offsets {
            let data_slice = &mmap[offset as usize..];
